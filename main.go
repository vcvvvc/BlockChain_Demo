...
func getSocketAddress(socket *websocket.Conn) string {
	if socket.IsClientConn() {
		return strings.Replace(socket.LocalAddr().String(), "ws://", "", 1)
	} else {
		return socket.Request().RemoteAddr
	}
}

func handleAddPeer(w http.ResponseWriter, r *http.Request) {
	peer, err := getPeerFromRequest(r)
	if err != nil {
		w.WriteHeader(http.StatusGone)
	...
	connectToPeers([]string{peer})
}

func getPeerFromRequest(r *http.Request) (string, error) {
	var v struct {
...
func handleMessage(v *ResponseBlockchain, ws *websocket.Conn) {
	switch v.Type {
	case queryLatest:
		sendResponse(responseLatestMsg(), v, ws)
...
func sendResponse(bs []byte, v *ResponseBlockchain, ws *websocket.Conn) {
	v.Type = responseBlockchain
	log.Printf("responseMsg: %s\n", bs)
	ws.Write(bs)
}
...
func isValidNewBlock(nb, pb *Block) (ok bool) {
	return nb.Hash == calculateHashForBlock(nb) &&
		pb.Index+1 == nb.Index &&
...
func handleBlockchainResponse(msg []byte) {
	...
	if latestBlockReceived.Index > latestBlockHeld.Index {
		handleNewBlocks(latestBlockReceived, latestBlockHeld, receivedBlocks)
	} else {
		log.Println("received blockchain is not longer than current blockchain. Do nothing.")
	}
}

func handleNewBlocks(latestBlockReceived *Block, latestBlockHeld *Block, receivedBlocks []*Block) {
